...
func NewPartitionStatusMessage(length byte, acknowledgeRequired bool, data []byte) (Message, error) {
	...
	message := &PartitionStatusMessage{
		...
		BypassCodeRequired:       (data[1] >> (7 - 0)) & 0x1,
		FireTrouble:              (data[1] >> (7 - 1)) & 0x1,
		Fire:                     (data[1] >> (7 - 2)) & 0x1,
		PulsingBuzzer:            (data[1] >> (7 - 3)) & 0x1,
		TlmFaultMemory:           (data[1] >> (7 - 4)) & 0x1,
		Armed:                    (data[1] >> (7 - 6)) & 0x1,
		Instant:                  (data[1] >> (7 - 7)) & 0x1,

		PartitionConditionFlags2: data[2],
		PreviousAlarm:            (data[2] >> (7 - 0)) & 0x1,
		SirenOn:                  (data[2] >> (7 - 1)) & 0x1,
		SteadySirenOn:            (data[2] >> (7 - 2)) & 0x1,
		AlarmMemory:              (data[2] >> (7 - 3)) & 0x1,
		Tamper:                   (data[2] >> (7 - 4)) & 0x1,
		CancelCommandEntered:     (data[2] >> (7 - 5)) & 0x1,
		CodeEntered:              (data[2] >> (7 - 6)) & 0x1,
		CancelPending:            (data[2] >> (7 - 7)) & 0x1,

		PartitionConditionFlags3: data[3],
		SilentExitEnabled:        (data[3] >> (7 - 1)) & 0x1,
		Entryguard:               (data[3] >> (7 - 2)) & 0x1,
		ChimeModeOn:              (data[3] >> (7 - 3)) & 0x1,
		Entry:                    (data[3] >> (7 - 4)) & 0x1,
		DelayExpirationWarning: (data[3] >> (7 - 5)) & 0x1,
		Exit1: (data[3] >> (7 - 6)) & 0x1,
		Exit2: (data[3] >> (7 - 7)) & 0x1,

		PartitionConditionFlags4: data[4],
		LedExtinguish:            (data[4] >> (7 - 0)) & 0x1,
		CrossTiming:              (data[4] >> (7 - 1)) & 0x1,
		RecentClosingBeingTimed:  (data[4] >> (7 - 2)) & 0x1,
		ExitErrorTriggered:       (data[4] >> (7 - 4)) & 0x1,
		AutoHomeInhibited:        (data[4] >> (7 - 5)) & 0x1,
		SensorLowBattery:         (data[4] >> (7 - 6)) & 0x1,
		SensorLostSupervision:    (data[4] >> (7 - 7)) & 0x1,

		LastUserNumber: data[5],

		PartitionConditionFlags5:   data[6],
		ZoneBypassed:               (data[6] >> (7 - 0)) & 0x1,
		ForceArmTriggeredByAutoArm: (data[6] >> (7 - 1)) & 0x1,
		ReadyToArm:                 (data[6] >> (7 - 2)) & 0x1,
		ReadyToForceArm:            (data[6] >> (7 - 3)) & 0x1,
		ValidPinAccepted:           (data[6] >> (7 - 4)) & 0x1,
		ChimeOn:                    (data[6] >> (7 - 5)) & 0x1,
		ErrorBeep:                  (data[6] >> (7 - 6)) & 0x1,
		ToneOn:                     (data[6] >> (7 - 7)) & 0x1,

		PartitionConditionFlags6: data[7],
		Entry1:                     (data[7] >> (7 - 0)) & 0x1,
		OpenPeriod:                 (data[7] >> (7 - 1)) & 0x1,
		AlarmSentUsingPhoneNumber1: (data[7] >> (7 - 2)) & 0x1,
		AlarmSentUsingPhoneNumber2: (data[7] >> (7 - 3)) & 0x1,
		AlarmSentUsingPhoneNumber3: (data[7] >> (7 - 4)) & 0x1,
		CancelReportIsInTheStack:   (data[7] >> (7 - 5)) & 0x1,
		KeyswitchArmed:             (data[7] >> (7 - 6)) & 0x1,
		DelayTripInProgress:        (data[7] >> (7 - 7)) & 0x1,
	}

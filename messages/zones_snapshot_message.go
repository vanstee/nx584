...
const (
	...
	ZonesSnapshotMessageTransitionCapable = message.TransitionCapableYes
	ZonesSnapshotMessageAcknowledged      = message.AcknowledgedPossible
)
...
func NewZonesSnapshotMessage(length byte, acknowledgeRequired bool, data []byte) (message.Message, error) {
	if length != ZonesSnapshotMessageLength {
		return nil, fmt.Errorf("message length incorrect: expected %d, actual: %d", ZonesSnapshotMessageLength, length)
	}

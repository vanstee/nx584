...
const (
	...
	PartitionStatusMessageTransitionCapable = message.TransitionCapableYes
	PartitionStatusMessageAcknowledged      = message.AcknowledgedPossible
)
...
func NewPartitionStatusMessage(length byte, acknowledgeRequired bool, data []byte) (message.Message, error) {
	if length != PartitionStatusMessageLength {
		return nil, fmt.Errorf("message length incorrect: expected %d, actual: %d", PartitionStatusMessageLength, length)
	}

...
func NewZoneStatusMessage(length byte, acknowledgeRequired bool, data []byte) (Message, error) {
	...
	message := &ZoneStatusMessage{
		...
		Partition1Enable: (data[1] >> (7 - 0)) & 0x1,
		Partition2Enable: (data[1] >> (7 - 1)) & 0x1,
		Partition3Enable: (data[1] >> (7 - 2)) & 0x1,
		Partition4Enable: (data[1] >> (7 - 3)) & 0x1,
		Partition5Enable: (data[1] >> (7 - 4)) & 0x1,
		Partition6Enable: (data[1] >> (7 - 5)) & 0x1,
		Partition7Enable: (data[1] >> (7 - 6)) & 0x1,
		Partition8Enable: (data[1] >> (7 - 7)) & 0x1,

		ZoneTypeFlags1:  data[2],
		Fire:            (data[2] >> (7 - 0)) & 0x1,
		N24Hour:         (data[2] >> (7 - 1)) & 0x1,
		KeySwitch:       (data[2] >> (7 - 2)) & 0x1,
		Follower:        (data[2] >> (7 - 3)) & 0x1,
		EntryExitDelay1: (data[2] >> (7 - 4)) & 0x1,
		EntryExitDelay2: (data[2] >> (7 - 5)) & 0x1,
		Interior:        (data[2] >> (7 - 6)) & 0x1,
		LocalOnly:       (data[2] >> (7 - 7)) & 0x1,

		ZoneTypeFlags2:  data[3],
		KeypadSounder:   (data[3] >> (7 - 0)) & 0x1,
		YelpingSiren:    (data[3] >> (7 - 1)) & 0x1,
		SteadySiren:     (data[3] >> (7 - 2)) & 0x1,
		Chime:           (data[3] >> (7 - 3)) & 0x1,
		Bypassable:      (data[3] >> (7 - 4)) & 0x1,
		GroupBypassable: (data[3] >> (7 - 5)) & 0x1,
		ForceArmable:    (data[3] >> (7 - 6)) & 0x1,
		EntryGuard:      (data[3] >> (7 - 7)) & 0x1,

		ZoneTypeFlags3:   data[4],
		FastLoopResponse: (data[4] >> (7 - 0)) & 0x1,
		DoubleEolTamper:  (data[4] >> (7 - 1)) & 0x1,
		Trouble1:         (data[4] >> (7 - 2)) & 0x1,
		CrossZone:        (data[4] >> (7 - 3)) & 0x1,
		DialerDelay:      (data[4] >> (7 - 4)) & 0x1,
		SwingerShutdown:  (data[4] >> (7 - 5)) & 0x1,
		Restorable:       (data[4] >> (7 - 6)) & 0x1,
		ListenIn:         (data[4] >> (7 - 7)) & 0x1,

		ZoneConditionFlags1: data[5],
		Faulted:             (data[5] >> (7 - 0)) & 0x1,
		Tampered:            (data[5] >> (7 - 1)) & 0x1,
		Trouble2:            (data[5] >> (7 - 2)) & 0x1,
		Bypassed:            (data[5] >> (7 - 3)) & 0x1,
		Inhibited:           (data[5] >> (7 - 4)) & 0x1,
		LowBattery:          (data[5] >> (7 - 5)) & 0x1,
		LossOfSupervision:   (data[5] >> (7 - 6)) & 0x1,

		ZoneConditionFlags2: data[6],
		AlarmMemory:         (data[6] >> (7 - 0)) & 0x1,
		BypassMemory:        (data[6] >> (7 - 1)) & 0x1,
	}
